  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        navigate("/"); // Redirect if API request fails
      }
    };
          ...
          if (response.data.success) {
            ...
            setPitches(
              response.data.pitches
                ...
                .filter((pitch) => pitch.status === "Accepted")
            );
          }
        } else {
          // Fetch only the user's pitches if they are an Investor Seeker
          response = await axios.get(`http://localhost:8081/pitch/${user.id}`);
                  ...
                  <div className="card-body d-flex align-items-center">
                    ...
                    {user && user.Profile ? (
                      <img
                        ...
                        onClick={() =>
                          setSelectedImage(`http://localhost:8081${user.Profile}`)
                        }
                      />
                    ) : (
                    ...
                    {user ? (
                      <div className="Profile">
                        ...
                        <p>
                          <b className="text-primary">Username:</b> {user.Username}
                        </p>
                        <p>
              ...
              <div className="col-md-12">
                <div className="card shadow-sm">
                  <div className="card-body">
                    <h1 className="text-center text-primary mb-4">My Pitches</h1>
                    <hr />
                    <div className="row">
                            ...
                            <div className="card h-100">
                              {isImage(pitch.File) && (
                                <img
                                  ...
                                  onClick={() =>
                                    setSelectedImage(`http://localhost:8081/${pitch.File}`)
                                  }
                                />
                              )}
                              {isVideo(pitch.File) && (
                                <video
                                  ...
                                  onClick={() =>
                                    setSelectedVideo(`http://localhost:8081/${pitch.File}`)
                                  }
                                >
                                  <source
                                    src={`http://localhost:8081/${pitch.File}`}
