        ...
        if (response.data.success) {
          ...
          setPitches(
            response.data.pitches.map((pitch) => {
              ...
              const rejectedIds = pitch.Reject ? pitch.Reject.split(",") : [];

              return {
                ...pitch,
                status: acceptedIds.includes(userId)
                  ? "Accepted"
    ...
    <div class="card-body">
      ...
      <p>
        ...
        ${
          ...
          <button class="btn btn-link p-0" data-bs-toggle="collapse" data-bs-target="#collapse-${
            pitch.id
          }">
            Read More
          </button>
...
`;
  ...
  const handleReject = (pitchId) => {
    ...
    axios
      .put("http://localhost:8081/Reject", { userId: user.id, pitchId })
      .then((response) => {
        if (response.data.success) {
          console.log("Pitch rejected successfully");
          setPitches((prevPitches) =>
          ...
          console.error("Error rejecting pitch:", response.data.message);
        }
      })
      .catch((error) => {
  ...
  const filteredPitches = pitches
    .filter((pitch) => !pitch.status) // Exclude accepted/rejected pitches
    .filter((pitch) =>
