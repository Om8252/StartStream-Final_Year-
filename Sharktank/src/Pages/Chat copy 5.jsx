...
const handlePaymentRequestSubmit = async () => {
  ...
  try {
    ...
    let paymentCardHTML;
    if (user.role === "InvestorSeaker") {
      // For InvestorSeaker, show only the QR code and payment request
      paymentCardHTML = `
      ...
      paymentCardHTML = `
        <div style="border: 1px solid #ccc; border-radius: 8px; padding: 16px; max-width: 300px; margin: 0 auto;">
          ...
          <div style="margin-top: 16px;">
            <label class="text-light" for="Recipt">Payment</label>
            <input
              type="text"
              placeholder="Enter payment reference"
              style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 10px;"
            />
            <label class="text-light" for="Recipt">Transaction Recipt</label>
            <input
              type="file"
