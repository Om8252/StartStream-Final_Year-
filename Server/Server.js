...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    }else if (req.originalUrl.includes('/payment/:id')) {
      folderPath = './uploads/receipts'; // Store in the 'Profile' folder for user profile images
    } else {
...
app.get('/get-user/:id', (req, res) => {
  ...
  const query = "SELECT id, fname, mname, lname, email,Phone, role,ProfileImg as Profile,Username FROM users WHERE id = ?";
...
app.post('/appointments', (req, res) => {
  const { Sender_id, Recived_id, appointment_date, location, status } = req.body;
  const query = `
    INSERT INTO appointments (Sender_id, Recived_id, appointment_date, location, status)
    VALUES (?, ?, ?, ?, ?)
  `;
  db.query(query, [Sender_id, Recived_id, appointment_date, location, status], (err, results) => {
    if (err) {
...
//   const query = `
//     SELECT DISTINCT u.id, u.fname, u.mname, u.lname,u.ProfileImg,u.email, u.username,u.role
//     FROM message m
//     JOIN users u 
...
app.get('/pitch/:id', (req, res) => {
  ...
  const query = `
    SELECT 
      p.id, p.pitch_title, p.description, p.category, p.File,p.Accept,u.Phone,
...
io.on("connection", (socket) => {
  ...
  socket.on("updateMessage", ({ messageId, updatedContent, sender, receiver }) => {
    console.log(`Updating message with ID ${messageId} to: ${updatedContent}`);

    // Update message in MySQL database
    const updateQuery =
        "UPDATE message SET Content = ? WHERE id = ? AND Sender_id = ? AND Reciver_id = ?";

    db.query(updateQuery, [updatedContent, messageId, sender, receiver], (err, result) => {
        if (err) {
            ...
            if (result.affectedRows > 0) {
                ...
                console.log("No message found with the given ID or sender/receiver mismatch");
            }
        }
    });
});
...
app.put('/payment/:id', upload.single('receiptFile'), (req, res) => {
  const paymentId = req.params.id;
  const { amount } = req.body; // Get amount from frontend

  if (!req.file) {
    return res.status(400).json({ message: 'Receipt file is required', success: false });
  }

  // Construct the file path for storing in the database
  ...
  const updateQuery = `
    ...
    WHERE id = ?
  `;
  const values = [receiptUrl, amount, paymentId];

  db.query(updateQuery, values, (err, result) => {
    if (err) {
      console.error('Error updating payment:', err);
      return res.status(500).json({ message: 'Error updating payment, please try again.', success: false });
    }
