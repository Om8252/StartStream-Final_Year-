...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessage", ({ sender, receiver, message }) => {
    ...
    const insertQuery =
      "INSERT INTO message (Sender_id, Reciver_id, Content, timestamp) VALUES (?, ?, ?, NOW())";
    ...
    db.query(insertQuery, [sender, receiver, message], (err) => {
      if (err) {
        ...
        const newMessage = { Sender_id: sender, Reciver_id: receiver, Content: message, timestamp: new Date() };
...
// app.get('/chat/:userId1/:userId2', (req, res) => {
...
//       m.Content, 
//       m.timestamp, 
//       sender.ProfileImg AS sender_photo, 
//       receiver.ProfileImg AS receiver_photo
//     FROM message m
//     LEFT JOIN users sender ON m.Sender_id = sender.id
//     LEFT JOIN users receiver ON m.Reciver_id = receiver.id
//     WHERE 
//       (m.Sender_id = ? AND m.Reciver_id = ?) 
//       OR 
//       (m.Sender_id = ? AND m.Reciver_id = ?)
...
//   const query = `
//     INSERT INTO message (Sender_id, Reciver_id, Content, timestamp) 
//     VALUES (?, ?, ?, NOW())
//   `;
